...
console.log("=== ALL SHAPES IN SLIDE LAYOUT 18 ===");
const layoutEntry = zip.getEntry("ppt/slideLayouts/slideLayout18.xml");
if (layoutEntry) {
  ...
  while ((match = shapeRegex.exec(xml)) !== null) {
    ...
    if (solidFillMatch) {
      const srgbMatch = solidFillMatch[1].match(/<a:srgbClr\s+val="([0-9a-fA-F]{6})"/);
      console.log("Fill Color:", srgbMatch ? `#${srgbMatch[1]}` : "Scheme/Other");
    }
  }
}
